...
- `N` — 切换发言稿面板（演讲时显示每页备注）
- `D` — 切换文稿模式（所有幻灯片连续滚动）
- `P` — 切换演讲者模式（Presenter：计时 + 进度 + 下一篇预览）
- `F` — 全屏（Fullscreen API，无需 F11）
- `B` — 黑场（演讲中暂休）
- `A` — 自动播放（8s/张）
- `1` ~ `9` — 数字键跳到对应章节
- `ESC` — 智能退出（先关面板 → 退全屏 → 回首页）
- 顶部"发言稿" / "演讲" / "全屏" / "自动" 按钮
- 底部计时器（演讲计时）
- 顶栏章节指示（PROLOGUE / THESIS / CHARACTERS / ACT I-III / EPILOGUE）
- 右下角圆形进度环（实时翻页进度）
- 鼠标点击触发窑火粒子特效
- 顶部 **`↓ PPT`** / **`↓ ZIP`** — 一键下载可分发的资源

### v2 · 全面优化
- 24 张幻灯片均支持 5 段 stagger 进场动画（fade-up / fade / scale / right）
- 章节背景：每幕用不同色温的渐变背景（ACT I 偏冷 · ACT II 偏暖 · ACT III 偏火光）
- pull-quote 自动生长金色边条
- 章节切换时顶栏 ACT 标"闪烁 + 拉长"
- 移动端响应式（768px 断点）
- 1920px+ 投影大屏自动放大字号
- 文稿模式新增 ACT 标注 + 打印优化（A4 横向 · 浅色方案）
