...
flowchart TB
    subgraph Frontend["前端 (Vue3 SPA)"]
        ...
        Store["状态层<br/>Pinia / reactive store"]
...
- 状态管理：原生 `reactive` + `provide/inject`（轻量，工具数据/筛选/搜索/收藏均在 `composables/useTools.ts` 内）
- 图标：`lucide-vue-next`
...
interface AITool {
  ...
  size: 'square' | 'portrait' | 'landscape'  // 卡片跨列规则
}

type ToolCategory =
  | 'chat' | 'image' | 'code' | 'office' | 'video'
