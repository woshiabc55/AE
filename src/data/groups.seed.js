...
const RARITY_INDEX = { T1: 0, T2: 1, T3: 2, T4: 3, T5: 4, T6: 5 };

function buildPack(groupIdx, packIdx) {
  const r = rng(groupIdx * 1000 + packIdx * 31 + 7);
  ...
  const level = Math.min(90, Math.max(1, Math.floor(r() * (groupIdx + 1) * 14) + Math.floor(r() * 30)));
  ...
  const cost = Math.floor((RARITY_INDEX[rarity] + 1) ** 1.8 * (level * 4 + 30));
  const locked = r() > 0.92;
  const equipped = r() > 0.85;
  const description = DESCRIPTIONS[Math.floor(r() * DESCRIPTIONS.length)];
  return { id: `SK-${idNum}`, code, name, rarity, level, tags, cost, locked, equipped, description };
}
...
export const seedGroups = GROUP_DEFS.map((g, gi) => ({
