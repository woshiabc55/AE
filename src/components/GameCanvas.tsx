...
export function GameCanvas() {
  ...
  useEffect(() => {
    ...
    const scene = new GameScene(containerRef.current, selectedOp);
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-void-950">
      ...
      {gameState === "playing" && !locked && (
        ...
        >
          <p className="font-pixel text-sm text-tac-400 text-glow-tac animate-flicker mb-3">
            点击进入战场
          </p>
      ...
      {/* 暂停菜单 */}
      {gameState === "paused" && (
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-void-950/85">
          {!showSettings ? (
            <div className="flex w-64 flex-col items-center gap-3 animate-fade-in">
              <h2 className="font-pixel text-base text-tac-400 text-glow-tac mb-2">
                暂停
              </h2>
