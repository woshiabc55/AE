    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-8 card-paper p-10 relative">
          ...
          <div className="text-silk-300/50 text-xs font-seal tracking-widest mb-3">
            卷末 · 客官寄语
          </div>
          <p className="font-brush text-silk-100 text-3xl md:text-4xl leading-relaxed">
            「{msg}」
          </p>
              ...
              <button
                ...
                className={`px-3 py-1.5 text-xs font-brush border tracking-widest transition ${
                  msg === m
                    ...
                    : 'border-gold-700/30 text-silk-300/70 hover:border-gold-500'
                }`}
              >
                {m.slice(0, 8)}…
              </button>
          ...
          <p className="mt-4 text-silk-300/50 text-sm">
            {name ? (
              <span>
                谨以此卷，奉 <span className="text-cinnabar font-brush">{name || '客官'}</span> 阁下落印。
              </span>
            ) : (
              '留一处空，待客官自填。'
            )}
        ...
        <div className="md:col-span-4 flex flex-col gap-4">
            ...
            <div className="space-y-3">
              {items.slice(0, 3).map((it) => (
                <div key={it.id} className="flex items-center gap-3 text-silk-300/80 text-sm">
                  ...
                  <span className="text-silk-300/40 font-seal text-xs">
          ...
          <div className="card-paper p-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                {collection.slice(0, 6).map((id) => {
                  ...
                  return (
                    <div
                      key={id}
                      className="aspect-square border border-gold-700/30 grid place-items-center"
                      style={{ background: `${it.swatch}33` }}
                    >
            ...
            <button
              onClick={() => {
                collection.forEach((id) => toggleCollect(id))
              }}
              className="mt-4 text-xs text-silk-300/40 hover:text-cinnabar transition"
            >
              清空私藏
            </button>
          </div>
            ...
            <ul className="space-y-2 text-sm text-silk-300/80">
              {classics.map((c) => (
                <li key={c.id} className="flex items-center justify-between">
                  <span>{c.title}</span>
                  <span className="text-silk-300/40 font-seal text-[10px] tracking-widest">
                    {c.dynasty}
                  </span>
                </li>
              ))}
