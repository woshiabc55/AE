...
const categories: { key: Item['category'] | 'all'; label: string; desc: string; seal: string }[] = [
  ...
  { key: 'ink', label: '墨', desc: categoryMap.ink.desc, seal: categoryMap.ink.seal },
  { key: 'paper', label: '纸', desc: categoryMap.paper.desc, seal: categoryMap.paper.seal },
  { key: 'brush', label: '笔', desc: categoryMap.brush.desc, seal: categoryMap.brush.seal },
  { key: 'inkstone', label: '砚', desc: categoryMap.inkstone.desc, seal: categoryMap.inkstone.seal },
  { key: 'tea', label: '茶', desc: categoryMap.tea.desc, seal: categoryMap.tea.seal },
  { key: 'incense', label: '香', desc: categoryMap.incense.desc, seal: categoryMap.incense.seal },
  { key: 'curio', label: '玩', desc: categoryMap.curio.desc, seal: categoryMap.curio.seal },
]
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-stretch">
        ...
        <div className="md:col-span-5 flex flex-col gap-2">
          {categories.map((c, i) => (
            ...
            >
              <span className="chapter-num text-silk-300/40 text-xs w-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {items
              ...
              .map((it) => (
                <div
                  key={it.id}
                  className="aspect-square border border-gold-700/30 p-3 flex flex-col justify-between"
                  style={{
                    background: `linear-gradient(180deg, ${it.swatch}33, transparent)`,
                  }}
                >
