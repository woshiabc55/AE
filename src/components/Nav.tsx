    ...
    <header className="border-b-2 border-bone/30 sticky top-0 z-50 bg-ink/90 backdrop-blur">
      <div className="max-w-[1400px] mx-auto flex items-center justify-between px-6 py-4">
        ...
        <nav className="flex items-center gap-1 md:gap-2 font-mono text-sm">
          ...
          <NavLink to="/packs" className={({isActive}) => `px-2 md:px-3 py-1.5 transition-colors flex items-center gap-1 ${isActive ? 'text-volt' : 'hover:text-volt'}`}>
            <Boxes size={12} className="hidden md:inline" /> 包
          </NavLink>
          <NavLink to="/qa" className={({isActive}) => `px-2 md:px-3 py-1.5 transition-colors flex items-center gap-1 ${isActive ? 'text-pink' : 'hover:text-pink'}`}>
            <ShieldCheck size={12} className="hidden md:inline" /> QA
          </NavLink>
          <NavLink to="/manifesto" className={({isActive}) => `px-2 md:px-3 py-1.5 transition-colors flex items-center gap-1 ${isActive ? 'text-volt' : 'hover:text-volt'}`}>
            <ScrollText size={12} className="hidden md:inline" /> 法典
          </NavLink>
          ...
          <NavLink to="/plum" className={({isActive}) => `px-2 md:px-3 py-1.5 transition-colors flex items-center gap-1 ${isActive ? 'text-cyan' : 'hover:text-cyan'}`}>
            <Grid3x3 size={12} className="hidden md:inline" /> 梅
          </NavLink>
