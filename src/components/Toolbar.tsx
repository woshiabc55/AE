...
export default function Toolbar() {
  ...
  return (
    <div className="flex items-center gap-1 px-3 py-1.5 border-b border-[#d4d4d4] bg-[#f8faf8]">
      <div className="flex items-center gap-0.5">
        <button
          onClick={() => setSelectionFormat({ bold: !format?.bold })}
          className={`p-1.5 rounded hover:bg-[#e8f5e9] transition-colors ${
            format?.bold ? 'bg-[#d4edda] text-[#1B4332]' : 'text-[#555]'
          }`}
          title="加粗"
        >
          <Bold size={16} />
        ...
        <button
          ...
          className={`p-1.5 rounded hover:bg-[#e8f5e9] transition-colors ${
            format?.italic ? 'bg-[#d4edda] text-[#1B4332]' : 'text-[#555]'
          }`}
          title="斜体"
        >
          <Italic size={16} />
      ...
      <div className="flex items-center gap-0.5">
        <button
          ...
          className={`p-1.5 rounded hover:bg-[#e8f5e9] transition-colors ${
            format?.align === 'left' ? 'bg-[#d4edda] text-[#1B4332]' : 'text-[#555]'
          }`}
          title="左对齐"
        >
          <AlignLeft size={16} />
        ...
        <button
          ...
          className={`p-1.5 rounded hover:bg-[#e8f5e9] transition-colors ${
            format?.align === 'center' ? 'bg-[#d4edda] text-[#1B4332]' : 'text-[#555]'
          }`}
          title="居中"
        >
          <AlignCenter size={16} />
        ...
        <button
          ...
          className={`p-1.5 rounded hover:bg-[#e8f5e9] transition-colors ${
            format?.align === 'right' ? 'bg-[#d4edda] text-[#1B4332]' : 'text-[#555]'
          }`}
          title="右对齐"
        >
          <AlignRight size={16} />
      ...
      <div className="flex items-center gap-0.5 relative" ref={bgRef}>
        <button
          ...
          className="p-1.5 rounded hover:bg-[#e8f5e9] transition-colors text-[#555] flex items-center gap-1"
          title="背景色"
        >
          <Palette size={16} />
          <span
            className="w-3.5 h-3.5 rounded-sm border border-[#ccc]"
            style={{ backgroundColor: format?.bgColor || '#ffffff' }}
          />
        </button>
        {showBgPicker && (
      ...
      <div className="flex items-center gap-0.5 relative" ref={textRef}>
        ...
        >
          ...
          <span
            className="w-3.5 h-3.5 rounded-sm border border-[#ccc]"
            style={{ backgroundColor: format?.textColor || '#2D3436' }}
          />
        </button>
        {showTextPicker && (
          <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-xl border border-[#e0e0e0] p-2 z-50 flex gap-1 flex-wrap w-[180px]">
            {TEXT_COLORS.map((color, i) => (
              <button
                ...
                className="w-6 h-6 rounded border border-[#ddd] hover:scale-110 transition-transform"
                style={{ backgroundColor: color || '#2D3436' }}
