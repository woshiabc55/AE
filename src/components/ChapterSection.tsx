...
export const ChapterSection: React.FC<Props> = ({ index, chapter, active, onPrev, onNext, progress }) => {
  return (
    <section id={`chapter-${chapter.id}`} className="relative h-screen w-full overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <h2 className="font-brush text-silk-100 text-5xl md:text-6xl mt-1">{chapter.title}</h2>
              <div className="text-silk-300/70 tracking-[0.4em] text-xs mt-2 font-seal">{chapter.subtitle}</div>
            </div>
          </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 caption-bar">
        <div className="max-w-5xl mx-auto px-8 py-8 flex items-end justify-between gap-6">
          <motion.div key={chapter.id} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="font-brush text-silk-100/90 text-2xl md:text-3xl">
          ...
          <div className="hidden md:flex flex-col items-end gap-2 shrink-0">
            <div className="flex items-center gap-2">
              <button onClick={onPrev} className="w-9 h-9 grid place-items-center border border-gold-500/30 text-gold-500/80 hover:bg-gold-500/10" aria-label="上一卷">←</button>
              <button onClick={onNext} className="w-9 h-9 grid place-items-center border border-gold-500/30 text-gold-500/80 hover:bg-gold-500/10" aria-label="下一卷">→</button>
            </div>
            <div className="text-[10px] text-silk-300/40 tracking-widest font-seal">◀ PREV · NEXT ▶</div>
          </div>
        </div>
      </div>
