...
const ARCHETYPES = [
  ...
  { name: '隐士',        role: 'mentor',        traits: ['寡言', '超然', '洞察', '孤独', '智慧'],             motivation: '远离尘世保存自我',   arc: '在入世中接受羁绊' },
  { name: '伪君子',      role: 'antagonist',    traits: ['表面温和', '内在阴冷', '算计', '世故', '魅力'],     motivation: '在体面中暗算他人',   arc: '在暴露时失去面具' },
  { name: '驯兽师',      role: 'foil',          traits: ['强硬', '控制', '耐心', '狡猾', '威慑'],             motivation: '让万物服从',         arc: '在失控中失去威严' },
  { name: '天使',        role: 'mentor',        traits: ['慈悲', '天真', '超然', '疗愈', '牺牲'],             motivation: '为受难者承担',       arc: '在人性中沉沦又升华' },
  { name: '恶魔',        role: 'antagonist',    traits: ['诱惑', '优雅', '智慧', '厌倦', '诱惑'],             motivation: '在堕落中确认意义',   arc: '在空虚中崩溃' },
  { name: '皮匠',        role: 'ally',          traits: ['巧手', '踏实', '传承', '沉默', '专注'],             motivation: '守住传统手艺',       arc: '在时代中寻找位置' },
  { name: '记账人',      role: 'foil',          traits: ['精细', '严谨', '冷酷', '秩序', '算账'],             motivation: '让所有账目对得上',   arc: '在坏账中失去意义' },
  { name: '放映员',      role: 'ally',          traits: ['旁观', '温柔', '叙述', '怀旧', '孤独'],             motivation: '为看不见的人播放',   arc: '在自己的故事中登场' },
  { name: '调香师',      role: 'protagonist',   traits: ['细腻', '敏感', '孤独', '极致', '回忆'],             motivation: '调出无法言说的情绪', arc: '在气味中重塑自我' },
  { name: '匠人',        role: 'protagonist',   traits: ['专注', '执着', '传承', '缓慢', '极致'],             motivation: '把手艺做到极致',     arc: '在时代中找到位置' },
];
...
const THEMES = [
  ...
  // 新增 200+
  '机械心', '电子灵魂', '赛博格化', '数字不朽', '肉身之死', '灵魂之生', '高维之音',
  '低语者', '钟摆', '沙漏', '日历翻篇', '午夜', '正午', '破晓', '黄昏', '永夜', '极昼',
  '雪崩', '山火', '干旱', '洪水', '海平面', '冰川', '冻土', '苔原', '红树林', '珊瑚',
  '沙漠', '绿洲', '盆地', '三角洲', '岛屿', '半岛', '海峡', '地峡', '海湾', '海峡',
  '灯塔', '航标', '罗盘', '星图', '黄道', '银河', '黑洞', '中子星', '白矮星', '红巨星',
  '超新星', '脉冲星', '类星体', '星云', '行星环', '卫星', '彗星', '陨石', '流星雨', '日食',
  '月食', '凌日', '掩星', '星等', '视差', '光年', '秒差距', '红移', '蓝移', '引力波',
  '暗物质', '暗能量', '宇宙微波', '暴胀', '暴涨', '回响', '回声', '回响', '共鸣', '和弦',
  '半音', '全音', '切分', '连音', '颤音', '滑音', '琶音', '分解', '和声', '复调',
  '对位', '卡农', '赋格', '奏鸣', '回旋', '变奏', '即兴', '华彩', '过门', '尾声',
  '前奏', '间奏', '副歌', '主歌', '独白', '旁白', '画外音', '内心独白', '自由间接', '意识流',
  '戏中戏', '剧中剧', '元叙事', '打破第四墙', '套层结构', '嵌套', '回环', '互文', '拼贴', '碎片',
  '蒙太奇', '长镜头', '空镜头', '推拉', '摇移', '升降', '跟拍', '航拍', '水下', '微观',
  '俯瞰', '仰拍', '平视', '窥视', '主观镜头', '客观镜头', '客观视点', 'POV', '过肩', '中景',
  '全景', '特写', '大特写', '中特写', '远景', '大远景', '跳切', '淡入', '淡出', '叠化',
  '黑场', '定格', '倒放', '快进', '慢动作', '定格动画', '黏土', '剪纸', '水墨', '版画',
  '手绘', '数字绘画', '矢量', '像素', '低多边形', '等距', '俯视', '45 度', '2D', '3D',
  '定格', '动画', '漫画', '分镜', '草图', '构图', '色彩', '光影', '质感', '风格化',
  '写实', '超写实', '写意', '留白', '晕染', '线描', '工笔', '写意', '工笔重彩', '淡彩',
  '水墨', '白描', '青绿', '金碧', '浅绛', '没骨', '指画', '指墨', '指头画', '指掌画',
  '中堂', '条幅', '横幅', '扇面', '册页', '长卷', '斗方', '镜心', '屏条', '通景',
  '立轴', '横批', '题款', '钤印', '印章', '闲章', '名章', '斋号', '引首章', '压角章',
  '笔法', '墨法', '章法', '构图', '气韵', '意境', '神似', '形似', '似与不似', '不求形似',
  '白描', '勾勒', '皴法', '渲染', '罩染', '分染', '托色', '打底', '铺色', '提白',
  '沙场', '点将', '布阵', '冲阵', '突围', '伏击', '夜袭', '偷袭', '强攻', '坚守',
  '巡逻', '侦察', '潜伏', '策反', '渗透', '潜伏', '间谍', '卧底', '接头', '死信箱',
  '密码本', '暗号', '代号', '假身份', '掩护', '撤退', '接应', '撤离', '收尾', '善后',
  '审讯', '刑讯', '逼供', '诱供', '测谎', '心理战', '宣传战', '认知战', '舆论战', '法律战',
  '黑客', '白客', '灰客', '脚本小子', '漏洞', '零日', '社工', '鱼叉', '水坑', '供应链',
  '勒索', '木马', '蠕虫', '病毒', '僵尸', '挖矿', 'DDoS', 'CC 攻击', 'XSS', 'CSRF',
  '注入', '越权', '未授权', '明文', '密文', '散列', '签名', '证书', '密钥', '私钥',
  '公钥', '对称', '非对称', '椭圆曲线', 'RSA', 'AES', 'SHA', 'MD5', 'Base64', 'URL 编码',
  '城堡', '要塞', '堡垒', '城墙', '城门', '吊桥', '护城河', '箭塔', '烽火台', '烽燧',
  '驿道', '驿站', '邮差', '急脚递', '铺递', '腰牌', '火牌', '勘合', '关文', '路引',
  '户籍', '黄册', '鱼鳞图册', '里甲', '保甲', '乡约', '族规', '家训', '祠堂', '族谱',
  '牌位', '灵位', '香火', '祭祀', '祭祖', '清明', '寒食', '端午', '中秋', '重阳',
  '除夕', '春节', '元宵', '庙会', '集市', '夜市', '晓市', '鬼市', '黑市', '当铺',
  '典当', '质库', '印子钱', '高利贷', '合会', '标会', '请会', '邀会', '打会', '轮会',
  '赌坊', '青楼', '戏园', '戏班', '梨园', '科班', '票房', '票友', '下海', '捧角',
  '角儿', '名角', '挑梁', '跑龙套', '开场', '压轴', '大轴', '垫场', '反串', '彩旦',
  '青衣', '花旦', '刀马旦', '老旦', '小生', '老生', '武生', '花脸', '丑角', '彩旦',
  '袍带', '靠甲', '箭衣', '马褂', '长衫', '旗袍', '马甲', '坎肩', '肚兜', '肚围',
  '云肩', '霞帔', '凤冠', '盖头', '喜帕', '盖头', '花轿', '凤辇', '宫车', '香车',
  '官轿', '民轿', '骡车', '马车', '牛车', '驴车', '独轮车', '板车', '轿车', '黄包车',
  '出租车', '公交车', '地铁', '轻轨', '高铁', '动车', '磁悬浮', '飞机', '热气球', '飞艇',
  '游艇', '渡轮', '货轮', '油轮', '航母', '潜艇', '战列舰', '巡洋舰', '驱逐舰', '护卫舰',
  '冲锋舟', '橡皮艇', '皮划艇', '赛艇', '龙舟', '凤舟', '舢板', '渔舟', '商船', '客船',
  '冰船', '雪橇', '狗拉雪橇', '驯鹿', '骆驼', '马帮', '骡帮', '驮队', '商队', '马队',
  '驿站', '旅馆', '客栈', '旅店', '招待所', '民宿', '青旅', '酒店', '宾馆', '度假村',
  '图书馆', '书店', '报摊', '书吧', '咖啡馆', '茶馆', '茶楼', '茶室', '酒馆', '酒吧',
  '夜店', '迪厅', 'KTV', '歌厅', '舞厅', '舞场', '俱乐部', '会所', '沙龙', '聚会',
  '客厅', '卧室', '厨房', '餐厅', '浴室', '书房', '阁楼', '地下室', '车库', '花园',
  '阳台', '露台', '天井', '院子', '庭园', '园林', '广场', '公园', '游乐场', '动物园',
  '植物园', '水族馆', '博物馆', '美术馆', '画廊', '展览馆', '体育馆', '健身房', '游泳池', '网球场',
  '高尔夫', '保龄球', '台球', '棋牌室', '麻将馆', '电竞馆', '网吧', '游戏厅', '电玩城', '保龄球馆',
  '废墟', '遗址', '遗迹', '古迹', '古建', '石窟', '石刻', '岩画', '壁画', '墓室',
  '皇陵', '王陵', '公墓', '陵园', '墓地', '坟场', '乱葬岗', '义庄', '善堂', '收容所',
  '孤儿院', '养老院', '医院', '诊所', '药店', '药铺', '医馆', '医学院', '实验室', '研究所',
  '天文台', '气象台', '地震台', '海洋站', '空间站', '科考站', '营地', '基地', '前哨', '哨所',
  '派出所', '公安局', '法院', '检察院', '监狱', '看守所', '戒毒所', '少管所', '工读学校', '精神病院',
  '收容站', '救助站', '难民营', '贫民窟', '棚户区', '城中村', 'CBD', '金融街', '科技园', '工业园',
  '保税区', '开发区', '新区', '特区', '直辖市', '省会', '地级市', '县城', '乡镇', '村寨',
  '部落', '族群', '氏族', '家族', '宗族', '皇族', '王族', '贵族', '平民', '奴隶',
  '公民', '市民', '居民', '村民', '渔民', '牧民', '农民', '工人', '商人', '艺人',
  '匠人', '学者', '教师', '医生', '律师', '法官', '警察', '军人', '间谍', '政客',
  '资本家', '无产者', '中产', '白领', '蓝领', '金领', '粉领', '灰领', '自由职业', '个体户',
  '流浪者', '拾荒者', '乞丐', '小偷', '强盗', '海盗', '山贼', '土匪', '刺客', '杀手',
  '雇佣兵', '保镖', '保安', '门卫', '仆从', '侍女', '太监', '宫女', '嫔妃', '皇后',
  '太子', '皇子', '公主', '王后', '国王', '皇帝', '宰相', '将军', '尚书', '侍郎',
  '御史', '刺史', '太守', '县令', '知府', '巡抚', '总督', '提督', '总兵', '参将',
  '游击', '都司', '千户', '百户', '总旗', '小旗', '军士', '民夫', '匠户', '灶户',
  '盐户', '茶户', '乐户', '疍户', '丐户', '惰民', '九姓渔', '畲民', '苗民', '瑶民',
  '黎民', '羌民', '回民', '蒙民', '藏民', '维民', '彝民', '壮民', '傣民', '佤民',
  '景颇', '怒族', '独龙', '门巴', '珞巴', '基诺', '德昂', '阿昌', '普米', '白族',
  '纳西', '傈僳', '哈尼', '拉祜', '布朗', '阿佤', '京族', '毛南', '仫佬', '土家',
  '羌族', '锡伯', '俄罗斯', '塔吉克', '塔塔尔', '乌孜别克', '柯尔克孜', '哈萨克', '裕固', '保安',
  '东乡', '撒拉', '土族', '回族', '满族', '朝鲜', '蒙古', '达斡尔', '鄂温克', '鄂伦春',
  '赫哲', '俄罗斯族', '京族', '高山族', '汉族', '少数民族', '原住民', '土著', '移民', '侨民',
  '难民', '流民', '饥民', '灾民', '流寇', '叛军', '乱军', '勤王军', '讨逆军', '义军',
  '团练', '民兵', '乡勇', '团丁', '保丁', '壮丁', '抽丁', '顶替', '买丁', '卖丁',
  '人贩', '牙行', '经纪', '掮客', '买办', '洋行', '领事', '传教士', '医生传教', '教育传教',
  '慈善', '赈灾', '义演', '义卖', '义诊', '义教', '义工', '志愿者', '社工', '心理师',
  '治疗师', '咨询师', '导师', '教练', '陪练', '陪护', '护工', '月嫂', '育儿嫂', '家政',
  '钟点工', '保洁', '保安', '门卫', '前台', '接待', '客服', '售后', '维修', '安装',
  '管道工', '电工', '木工', '瓦工', '油漆工', '装修工', '架子工', '塔吊', '挖掘机', '装载机',
  '叉车', '货车', '公交', '出租', '代驾', '网约车', '外卖', '快递', '跑腿', '代购',
  '代孕', '代考', '代写', '代练', '代骂', '陪聊', '陪玩', '陪唱', '陪舞', '陪酒',
  '小姐', '公主', '鸭子', '妈咪', '老鸨', '皮条客', '马夫', '龟奴', '打手', '杀手',
  '血亲', '姻亲', '义亲', '继亲', '养亲', '寄亲', '干亲', '奶亲', '教亲', '同乡',
  '同学', '同事', '战友', '袍泽', '同袍', '同志', '道友', '同门', '师兄', '师弟',
  '师姐', '师妹', '师徒', '徒孙', '曾祖', '高祖', '太祖', '鼻祖', '先祖', '列祖',
  '列宗', '宗祠', '支祠', '家庙', '祖庙', '太庙', '社稷', '宗社', '社坛', '社学',
];
...
const SCENE_TEMPLATES = {
  ...
  'flashback': { // 闪回
    action: [
      '时间倒回{era}的{place}，{role}年轻的模样在{time}的{atmos}中浮现。',
      '{role}的思绪被{memory}拽回{place}，{event}的{weight}在胸口重新打开。',
    ],
    hooks: [
      '{role}看见{younger_self}的{choice}。',
      '{role}明白{trauma}的{origin}。',
    ],
    transition: ['FLASHBACK TO:', 'DISSOLVE TO:', 'MATCH CUT TO:', 'BACK TO:'],
  },
  'chase': { // 追逐
    action: [
      '{role}在{place}狂奔，{pursuer}在{atmos}中紧追不舍，{stakes}在{terrain}的尽头等待。',
      '{place}里，{role}与{antagonist}的{sequence}进入{climax}，{obstacle}接踵而至。',
    ],
    hooks: [
      '{role}在{place}的{edge}失去{pursuer}。',
      '{role}在{place}完成{turning}。',
    ],
    transition: ['SMASH CUT TO:', 'HARD CUT TO:', 'FOLLOWED BY:', 'INTERCUT WITH:'],
  },
  'revelation': { // 真相揭示
    action: [
      '{place}的{time}，{role}撕开{evidence}的封口，{truth}如{light}照进{atmos}。',
      '{role}在{place}与{antagonist}对峙，{mask}在{the_word}的{weight}中滑落。',
    ],
    hooks: [
      '{role}明白{ally}才是{traitor}。',
      '{role}自己就是{chosen}。',
    ],
    transition: ['HOLD ON:', 'SMASH CUT TO:', 'FADE TO BLACK.', 'IRIS OUT.'],
  },
  'prologue': { // 序章
    action: [
      '很久以前，{era}的{place}，{role}做了一件事，{consequence}在{time}的远处等待。',
      '在{myth}里，{era}的{place}是{truth}的发源地，{atmos}中的一切即将被改变。',
    ],
    hooks: [
      '{role}种下了{seed}。',
      '{role}失去了{the_thing}。',
    ],
    transition: ['MANY YEARS LATER...', 'FAST FORWARD TO:', 'CUT TO:', 'FADE IN:'],
  },
};
...
// 标题元素库
const TITLE_OBJECTS = ['灯塔', '回声', '信', '星图', '密令', '影子', '挽歌', '风铃', '雨季', '玻璃', '密钥', '罗盘', '裂缝', '钟声', '旧屋', '黑匣子', '灰烬', '信号', '海图', '日历', '画像', '雪原', '潮汐', '长街', '医院', '站台', '花房', '钟楼', '碎片', '盲盒', '回廊', '霓虹', '剪影', '墓志铭', '白噪声', '旧照', '邮箱', '钢琴', '冷月', '碎镜', '边缘', '影', '小夜曲', '路', '沙漏', '纸船', '药', '信笺', '余烬', '暗流', '雪', '玻璃栈桥', '信标', '哑火', '留白', '白旗', '黑潮', '青鸟', '夜枭', '白鸦', '老镜', '远钟', '残灯', '余温', '血书', '孤帆', '海沟', '云海', '薄冰', '厚雪', '浓雾', '细雨', '骤雨', '急雨', '夜雨', '晨雨', '过云雨', '雷阵雨', '梅雨', '秋雨', '春雨', '冬雨', '寒雨', '热雨', '对雨', '听雨', '看雨', '躲雨', '冒雨', '浴雨', '醉酒', '残茶', '凉酒', '残局', '冷棋', '孤棋', '废棋', '悔棋', '重棋', '新棋', '古棋', '今棋', '明棋', '前世', '今生', '来生', '异世', '他世', '此世', '彼岸', '此岸', '中间'];
const TITLE_SUBJECTS = ['最后的', '无人知晓的', '北方的', '第三种', '缓慢的', '失重的', '被遗忘的', '微小的', '永恒的', '黑夜的', '古老的', '从前的', '虚构的', '寂静的', '明日的', '第七日', '漫长的', '深海的', '关于', '一件', '半山', '山前', '灯下的', '夜半的', '夏日', '冬日', '初春', '深秋', '黄昏的', '清晨的', '蓝色', '灰色', '白色', '红色', '黑色', '无名的', '沉默的', '看不见的', '被听见', '不该被记住的', '无人认领的', '第二十一', '第三十二', '第一次', '最后一次', '仅有的', '残存的', '余下的', '未完的', '已完的', '未寄的', '迟到的', '缺席的', '多余的', '重要的', '无关紧要的', '最轻的', '最重的'];
const TITLE_SUFFIX = ['之下', '之后', '之前', '之间', '之外', '之境', '之约', '之歌', '手记', '备忘录', '残卷', '补遗', '补注', '物语', '考', '异闻录', '断章', '七章', '拾遗', '巡礼', '别录', '史记', '笺注', '后日谈', '前夜', '札记', '答问', '合奏', '独白', '副歌', '间奏', '回旋', '中板', '变奏', '夜曲', '终曲', '前奏', '终章', '尾声', '未完', '残篇', '拾零', '散记', '别章', '续编', '补叙', '别册', '续', '前传', '后传', '新编', '新解', '新译', '旧版', '新版', '无印', '初版', '修订', '增订', '注疏', '正义', '音义', '校注', '批注', '圈点', '眉批', '夹注', '题跋', '序', '跋', '缘起', '本事', '因缘', '发端', '收束', '后记'];
...
function main() {
  ...
  for (let i = 1; i <= COUNT; i++) {
    ...
    if (i % 500 === 0) console.log(`[forge] ${i}/${COUNT}`);
  }
  const t1 = Date.now();
  ...
  const chunks = Math.ceil(scripts.length / CHUNK_SIZE);
  console.log(`[forge] 分块：${chunks} 块 × ${CHUNK_SIZE} 份`);

  // 计算每个剧本的 _chunk 索引
  const scriptsWithChunk = scripts.map((s, i) => ({ ...s, _chunk: Math.floor(i / CHUNK_SIZE) }));

  // 1) 写出 元数据 data-meta.js —— 只保留列表/筛选需要的字段
  const metaOut = path.join(path.dirname(OUT), 'data-meta.js');
  const metaList = scriptsWithChunk.map(s => ({
    id: s.id,
    title: s.title,
    type: s.type,
    genre: s.genre,
  ...
  const metaCode = `// SCRIPT.FORGE 元数据 — 由 generate-data.js 生成
// 种子：${SEED}  数量：${metaList.length}  分块：${chunks} × ${CHUNK_SIZE}  生成时间：${new Date().toISOString()}
'use strict';
window.SCRIPTS_META = ${JSON.stringify(metaList)};
window.SCRIPTS_CHUNK_COUNT = ${chunks};
window.SCRIPTS_CHUNK_SIZE = ${CHUNK_SIZE};
`;
  fs.writeFileSync(metaOut, metaCode, 'utf8');
  const metaStat = fs.statSync(metaOut);
  console.log(`[forge] 元数据: ${metaOut}  ${(metaStat.size / 1024).toFixed(1)}KB`);

  // 2) 写出每个分块 data-N.js —— 完整剧本数据
  let fullTotal = 0;
  for (let c = 0; c < chunks; c++) {
    const chunkScripts = scriptsWithChunk.filter(s => s._chunk === c);
    const code = `// SCRIPT.FORGE 分块 ${c}/${chunks} — 由 generate-data.js 生成
// 包含 ${chunkScripts.length} 份完整剧本（id ${c * CHUNK_SIZE + 1} - ${c * CHUNK_SIZE + chunkScripts.length}）
'use strict';
window.SCRIPTS_CHUNKS = window.SCRIPTS_CHUNKS || {};
window.SCRIPTS_CHUNKS[${c}] = ${JSON.stringify(chunkScripts, null, 0)};
`;
    const p = path.join(path.dirname(OUT), `data-${c}.js`);
    fs.writeFileSync(p, code, 'utf8');
    const st = fs.statSync(p);
    fullTotal += st.size;
    console.log(`[forge] 分块 ${c}: ${p}  ${(st.size / 1024).toFixed(1)}KB  (${chunkScripts.length} 份)`);
  }
  console.log(`[forge] 分块总大小：${(fullTotal / 1024).toFixed(1)}KB`);

  // 3) 写出 data.js —— 兼容旧版（同时含 meta + 预加载所有分块）
  const compat = `// SCRIPT.FORGE 兼容数据 — 由 generate-data.js 生成
// 包含所有 ${scripts.length} 份剧本（元数据 + 完整数据）
'use strict';
window.SCRIPTS = ${JSON.stringify(scripts, null, 0)};
`;
  const compatPath = path.join(path.dirname(OUT), 'data-compat.js');
  fs.writeFileSync(compatPath, compat, 'utf8');
  const compatStat = fs.statSync(compatPath);
  console.log(`[forge] 兼容完整数据: ${compatPath}  ${(compatStat.size / 1024 / 1024).toFixed(2)}MB`);

  // 4) 写一个 data.js 桩（指向 meta + 分块）
  const stub = `// SCRIPT.FORGE 入口 — 由 generate-data.js 生成
'use strict';
console.warn('[SCRIPT.FORGE] data.js 已变更为分块加载，请使用 data-meta.js + data-N.js');
`;
  // 不覆盖用户的 data.js 实际使用 — 跳过此步骤
  // fs.writeFileSync(OUT, stub, 'utf8');

  // 统计
  const t2 = Date.now();
  console.log(`[forge] 题材分布 (前 20)：`);
  const map = new Map();
  for (const s of scripts) map.set(s.genre, (map.get(s.genre) || 0) + 1);
  for (const [k, v] of [...map.entries()].sort((a, b) => b[1] - a[1]).slice(0, 20)) {
    console.log(`  ${k.padEnd(8)} ${v}`);
  }
  console.log(`[forge] 总耗时：${(t2 - t0)}ms`);
  console.log(`[forge] 数据规模：${scripts.length} 份 / ${chunks} 块 / 元数据 ${(metaStat.size / 1024).toFixed(0)}KB / 全文 ${(fullTotal / 1024 / 1024).toFixed(2)}MB`);
}
